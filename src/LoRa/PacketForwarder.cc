...
void PacketForwarder::handleMessage(cMessage* msg) {
  ...
  if (msg->arrivedOn("lowerLayerIn")) {
    ...
    // FIXME : debug for now to see if LoRaMAC frame received correctly from
    // network server
...
void PacketForwarder::processLoraMACPacket(Packet* pk) {
  ...
  // for (std::vector<nodeEntry>::iterator it = knownNodes.begin() ; it !=
  // knownNodes.end(); ++it)
...
void PacketForwarder::receiveSignal(cComponent* source,
...
void PacketForwarder::finish() {
  recordScalar("LoRa_GW_DER", double(counterOfReceivedPackets) /
                                  counterOfSentPacketsFromNodes);
}
